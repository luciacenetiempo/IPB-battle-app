...
class SSESocket {
    ...
    updateTimerPolling(state) {
        ...
        if (needsTimerPolling && !this.timerPollingInterval) {
            // Avvia polling ogni 500ms per il timer (più frequente per catturare il momento esatto)
            this.timerPollingInterval = setInterval(() => {
                if (this.currentState) {
                    ...
                    if (this.currentState.isTimerRunning && this.currentState.timerStartTime) {
                        ...
                        // Lo chiamiamo ogni 500ms quando il timer è attivo e rimane poco tempo (<= 5 secondi)
                        // Questo assicura che la generazione venga triggerata anche se c'è un piccolo delay
                        if (timer <= 5 && status === 'WRITING' && (this.lastCheckTimerCall === 0 || Date.now() - this.lastCheckTimerCall > 500)) {
                            this.lastCheckTimerCall = Date.now();
                            console.log('[SSESocket] Calling check-timer, timer:', timer, 'status:', status);
                            fetch('/api/check-timer')
                                .then(res => {
                                    console.log('[SSESocket] check-timer response status:', res.status);
                                    return res.json();
                                })
                                .then(data => {
                                    console.log('[SSESocket] check-timer response data:', data);
                                    if (data.message === 'Generation triggered') {
                                        console.log('[SSESocket] Generation triggered successfully');
                                    } else {
                                        console.log('[SSESocket] Generation not triggered. Status:', data.status, 'Timer:', data.timer, 'isTimerRunning:', data.isTimerRunning, 'generationTriggered:', data.generationTriggered);
                                    }
                                })
                                .catch(err => {
    ...
    async emit(event, data) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error('[SSESocket] Event response error:', response.status, errorData);
                
                // Emit error events per participant:join
                if (event === 'participant:join') {
                    const listeners = this.listeners.get('error:join') || [];
                    // Se è un errore di rejoin non autorizzato, invia un messaggio più chiaro
                    if (errorData.error === 'UNAUTHORIZED_REJOIN') {
                        listeners.forEach(cb => cb('TOKEN_ALREADY_IN_USE'));
                    } else {
                        listeners.forEach(cb => cb(errorData.error || errorData.message || 'NETWORK_ERROR'));
                    }
                }
            ...
            // Se la risposta include uno stato, aggiorna currentState
            if (result.state) {
                console.log('[SSESocket] Updating currentState from response, validTokens:', result.state.validTokens, 'status:', result.state.status, 'round:', result.state.round);
                this.currentState = result.state;
                // Emetti anche un evento state:update locale se ci sono listener
                const listeners = this.listeners.get('state:update') || [];
                console.log('[SSESocket] Notifying', listeners.length, 'state:update listeners with tokens:', result.state.validTokens);
                listeners.forEach(cb => {
                    try {
                        cb(result.state);
                    } catch (error) {
                        console.error('[SSESocket] Error in state:update listener:', error);
                    }
                });
            }
